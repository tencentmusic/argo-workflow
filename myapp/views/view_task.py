...
class Task_ModelView_Base():
    ...
    def run_pod(self,task,k8s_client,run_id,namespace,pod_name,image,working_dir,command,args):
        ...
        def template_str(src_str):
            ...
            des_str = rtemplate.render(creator=task.pipeline.created_by.username,
                                       ...
                                       cluster_name=task.pipeline.project.cluster.get('NAME','dev')
                                       )
            return des_str
        # 全局环境变量
    ...
    def debug(self,task_id):
        ...
        return redirect("/myapp/web/debug/%s/%s/%s/%s"%(task.pipeline.project.cluster.get('NAME','dev'),namespace,pod_name,pod_name))
    ...
    def run_task(self,task_id):
        ...
        return redirect("/myapp/web/log/%s/%s/%s" % (task.pipeline.project.cluster.get('NAME','dev'),namespace, pod_name))
