...
def init_db():
    ...
    if SQLALCHEMY_DATABASE_URI:
        ...
        sql = "CREATE DATABASE IF NOT EXISTS pipeline DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;"
        # 执行创建数据库的sql
        cursor.execute(sql)
        conn.commit()

        sql = "CREATE DATABASE IF NOT EXISTS argo DEFAULT CHARACTER SET utf8 DEFAULT COLLATE utf8_general_ci;"
        # 执行创建数据库的sql
        cursor.execute(sql)
        conn.commit()
